(function() {
  ...
  /*
  * Modified by: Tania Minkova
  * Date: 20.07.2015
  * JS code to go live pizza page on 60+ FPS
  * */
  ...
  var resizePizzas = function(size) {
    ...
    // Use DOM access once to populate element array
  ...
  // Use pre-calculated values instead of calculate on each iteration
  // Use movers array to avoid DOM access
  function updatePositions() {
    ...
    var phaseParts = [
      Math.sin(topScroll)*100,
      Math.sin(topScroll+1)*100,
      Math.sin(topScroll+2)*100,
      Math.sin(topScroll+3)*100,
      Math.sin(topScroll+4)*100
    ];
    var phasePartIndex = 0;
    ...
    for (var i = 0; i < len; i++) {
      ...
      movers[i].style.left = movers[i].basicLeft + phaseParts[phasePartIndex] + 'px';
      phasePartIndex++;
    }
  ...
  // Populate movers array to not use DOM access each time
  // Reduced number of mover elements from 200 to 48
  document.addEventListener('DOMContentLoaded', function() {
    var cols = 8;
    var s = 256;
    var rootElement = document.getElementById("movingPizzas1");
    var elem;
    for (var i = 0; i < 48; i++) {
